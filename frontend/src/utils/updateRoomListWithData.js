...
export async function updateRoomListWithData(data) {
  ...
  if (data) {
    if (data.is_room_details) rooms = updateRoomListWithNewDetails(data)
    else if (data.is_room_delete) rooms = updateRoomListWithDeletedRoomId(data)
    else if (data.is_message_update) rooms = updateRoomListWithMessage(data, false)
    else if (data.is_tag_rename) rooms = updateRoomListWithTag(data)
    else {
      // Если была создана комната только что и пришло системное сообщение в ней
      if (!roomsStore.rooms.some((item) => item.id === data.room_id)) return

      rooms = updateRoomListWithMessage(data)
    }
  } else rooms = await loadRoomList() // забираем с сервера актуальный счетчик, в списке комнат
